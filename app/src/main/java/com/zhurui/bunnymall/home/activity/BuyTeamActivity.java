...
public class BuyTeamActivity extends BaseActivity {
    ...
    private void initView(){
        img_banner.setBackgroundResource(R.drawable.banner_normal);
        text_title.setText(R.string.team_shop);
        ...
        buyLimitAdapter = new BuyLimitAdapter(BaseApplication.getInstance(),true);
        buyLimitAdapter.setOnItemClickLitener(new BuyLimitAdapter.OnItemClickLitener() {
            @Override
            public void onItemClick(View view, int position) {
                startActivity(new Intent(BaseApplication.getInstance(),ProductNewDetailActivity.class).putExtra("productType",3).putExtra("productId",Long.parseLong(buyLimitAdapter.preferentialBeanList.get(position).getProductID())),false);
            }
...
//                4;已结束  3;团购中  2;即将开始
                if(position<3){
                    grouppurstatus = 4;//已结束
                }else if(3== position){
                    grouppurstatus = 3;//团购中
                }else {
                    grouppurstatus = 2;//即将开始
                }
...
//        getData(grouppurstatus, TimeUtils.getCurrentTimeInString(TimeUtils.DATE_FORMAT_DATE));
    ...
    private void getData(int grouppurstatus,String timequery){
        ...
        try {
            ...
            jsonObject.put("groupbuytypeid","2");
        ...
        mHttpHelper.post(Contants.BASE_URL, params, new SpotsCallBack<PreferentialRespMsg>(this) {
            ...
            public void onSuccess(Response response, PreferentialRespMsg preferentialRespMsg) {
                if (preferentialRespMsg.getResult() > 0) {
                    ...
                    ToastUtils.show(BuyTeamActivity.this, preferentialRespMsg.getRetmsg());
                }
            }
            @Override
            public void onError(Response response, int code, Exception e) {
                ToastUtils.show(BuyTeamActivity.this, "请求失败，请稍后重试");
            }
