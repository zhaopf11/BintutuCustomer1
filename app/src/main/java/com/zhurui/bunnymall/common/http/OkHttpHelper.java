                    ...
                    Log.e("resultStr返回数据 ：",resultStr.toString());
                      if (callback.mType == String.class) {
                        ...
                        if (response.request().url().toString().contains(Contants.BASE_URL)) {
                            try {//尝试解析为基础数据
                                if(Contants.WECHAT_PAY){
                                    WeChatRespMsg respMsg = mGson.fromJson(resultStr,WeChatRespMsg.class);
                                    if (respMsg != null) {
                                        if (respMsg.getResult() == BaseRespMsg.TO_LOGIN) {
                                            BaseApplication.getInstance().clearUser();
                                            callback.mContext.startActivityForResult(new Intent(callback.mContext, LoginActivity.class).putExtra("TO_LOGIN",respMsg.getResult()),1);
                                        } else {
